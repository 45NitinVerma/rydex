...
function page() {
	...
	const [distToDrop, setDistToDrop] = useState(0);
		...
		return (
			<div className="h-screen w-full bg-zinc-950 flex items-center justify-center">
				<div className="flex flex-col items-center gap-4">
					...
					<Link href={'/'} className="text-zinc-900 bg-white py-2 px-6 rounded-full text-sm tracking-widest uppercase font-medium hover:bg-zinc-200 hover:text-zinc-100 transition-colors">
						Go Back
					</Link>
				</div>
			</div>
	...
	const cfg = STATUS_LABELS[booking?.bookingStatus! ?? "confirmed"];
	...
	const displayDist = status === "confirmed" ? distToPickUp : distToDrop;
			...
			<div className="relative flex-1 h-full z-0">
				...
				>
					<div className="flex items-center gap-2 bg-white/95 backdrop-blur-sm px-4 py-2 rounded-full shadow-lg border border-zinc-100">
						...
						/>
						<span className="text-xs font-semibold tracking-white text-zinc-900">
							{cfg.label}
						</span>
					</div>
				</motion.div>
						...
						<AnimatePresence mode="wait">
							{status === "confirmed" &&
								...
								!otpVerified && (
									...
									>
										<MapPin size={15} /> I've Arrived at
										Pickup{" "}
										<ArrowRight
											size={15}
											className="ml-1"
