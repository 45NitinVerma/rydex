...
const VEHICLES = [
	...
	{ id: "auto", label: "Auto", Icon: Car, desc: "Everday rides" },
...
function page() {
	...
	const [dropCountry, setDropCountry] = useState<string>("");
		...
		<div className="min-h-screen bg-zinc-100 flex items-center justify-center px-4 py-10">
			...
			>
				...
				<div className="bg-white rounded-3xl border border-zinc-200 shadow-[0_8px_40px_rgba(0,0,0,0.08)] overflow-visible">
					<div className="h-1 bg-zinc-900 w-[90%] m-auto rounded-t-3xl" />
					<div className="p-6 space-y-7">
                        ...
                        ${active ? "bg-zinc-900 border-zinc-900 shadow-lg" : "bg-zinc-50 border-zinc-200 hover:border-zinc-400"}`}
											onClick={() =>
												setVehicle(v.id as vehicleType)
											}
										>
											<div
						...
						>
							...
							<div className="flex items-center gap-3 bg-zinc-50 border border-zinc-200 rounded-2xl px-4 py-3 focus-within:border-zinc-900 focus-wihtin:bg-white transition-all">
						...
						>
							...
							<div className="bg-zinc-50 border border-zinc-200 rounded-2xl overflow-visible">
								<div className="relative z-30">
									<div className="flex items-centeer gap-3 px-4 py-3.5 focus-within:bg-white rounded-t-2xl transition-colors">
										...
										<motion.button
											...
											whileTap={{ scale: 0.88 }}
											disabled={locating}
											className={`w-8 h-8 rounded-xl bg-zinc-200 hover:bg-zinc-300 transition-col flex items-center justify-center flex-shrink-0 ${locating ? "cursor-not-allowed" : "cursor-pointer"}`}
										>
											<LocateFixed
												size={14}
												...
												{pickUpSuggestions.map(
													(p, i) => (
														...
														>
															...
															<ChevronRight
																size={13}
																className="text-zinc-300 ml-auto flex-shrink-0"
															/>
														</motion.div>
													),
												)}
								...
								<div className="relative z-10">
									<div className="flex items-centeer gap-3 px-4 py-3.5 focus-within:bg-white rounded-t-2xl transition-colors">
										...
										<input
											...
											className={`flex-1 bg-transparent text-sm font-semibold text-zinc-900 placeholder:text-zinc-400 outline-none ${!pickUpCountry ? "text-zinc-300 cursor-not-allowed" : ""}`}
										/>
										<Navigation
											size={14}
											className={`text-zinc-700 flex-shrink-0 ${!pickUpCountry ? "opacity-30 cursor-not-allowed" : ""}`}
										/>
									</div>
						...
						>
							<motion.button
								...
								onClick={() => {
									router.push(
										`/user/search?pickup=${encodeURIComponent(pickUp)}&drop=${encodeURIComponent(drop)}&vehicle=${vehicle}&mobile=${encodeURIComponent(mobile)}&pickuplat=${pickUpLat}&pickuplng=${pickUpLng}&droplat=${dropLat}&droplng=${dropLng}`,
									);
								}}
								className={`w-full h-14 rounded-2xl bg-zinc-900 hover:bg-black text-white font-black text-sm tracking-wide flex items-center justify-center gap-25 transition-colors shadow-lg disabled:shadow-none disabled:cursor-not-allowed ${canContinue ? "" : "opacity-30"}`}
							>
