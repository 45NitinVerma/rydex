...
function AdminEarning() {
	...
	const total = earningData.reduce((acc, curr) => acc + curr.earnings, 0);
	const avg = earningData.length
		? Number((total / earningData.length).toFixed(2))
		: 0;
	const max = earningData.length
		? Math.max(...earningData.map((e) => e.earnings))
		: 0;
	const bestDay = earningData.find((e) => e.earnings === max);
	...
	const deltaPct = yesterday
		? ((delta / yesterday.earnings) * 100).toFixed(1)
		: 0;
			...
			<div className="grid grid-cols-3 gap-3 mb-6">
				{metrics.map((m, i) => (
					...
					>
						...
						>
							<span
								className={`${m.bg} p-1 rounded-lg ${m.color}`}
							>
								{m.icon}
							</span>
							{m.label}
					...
					<ResponsiveContainer width="100%" height="100%">
						<BarChart data={earningData} barCategoryGap={"30%"}>
							...
							<Bar dataKey="earnings" radius={[8, 8, 3, 3]}>
								{earningData.map((d, i) => {
									const isToday =
										i === earningData.length - 1;
									const isBest =
										d.earnings === max && !isToday;
									return (
										<Cell
											key={`cell-${i}`}
											fill={
