...
const pickUpIcon = new L.DivIcon({
	html: `<div style="display:flex; flex-direction:column; align-items:center; filter:drop-shadow(0 4px 12px rgba(0,0,0,0,0.28))">
...
const driverIcon = new L.DivIcon({
	html: `<div id="car-marker" style="width:52px; height:52px; display:flex; align-items:center; justify-content:center; transform-origin:center; transition: transform 0.3s cubic-bezier(0.34,1.56,0.64,1); filter:drop-shadow(0 6px 18px rgba(0,0,0,0.5));
    ">
...
function LiveRideMap({
	...
	useEffect(() => {
		...
		const [pickLat, pickLng] = pickUpLocation
		...
		const fetchRoutes = async () => {
			try {
				if (mapStatus === "arriving") {
                    ...
                    onStats?.({
                        distToPickUp: (pickUpRoute?.distance ?? 0)/1000,
                        etaToPickUp: Math.ceil((pickUpRoute?.duration ?? 0)/60),
                        distToDrop: (dropRoute?.distance ?? 0)/1000,
                        etaToDrop: Math.ceil((dropRoute?.duration ?? 0)/60),
                    })
	...
	const showPickUpRoute =
		mapStatus === "arriving" && routeToPickUp.length > 0;
	...
	return (
		<div className="relative w-full h-full bg-zinc-100">
			...
			>
                ...
                {showDropRoute && (
                    <Polyline positions={routeToDrop} pathOptions={{
                        color:"#0a0a0a",
                        weight:4,
                        lineCap: "round",
                        lineJoin:"round"
