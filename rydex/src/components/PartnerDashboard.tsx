	...
	return (
		<div className="min-h-screen bg-linear-to-br from-gray-100 to-gray-200 px-4 pt-28 pb-20">
			<div className="max-w-7xl mx-auto space-y-16">
					...
					(userData?.videoKycStatus === "approved" ? (
					  ...
					  userData.videoKycRoomId ? (
						<ActionCard
							...
							onClick={() =>
								router.push(
									`/video-kyc/${userData.videoKycRoomId}`,
								)
							}
						/>
					) : (
				...
				{activeStep == 7 && vehicleData?.status === "pending" && (
					<StatusCard
						...
						desc={"Admin is verifying your pricing"}
					/>
				)}
				...
				{activeStep == 8 && vehicleData?.status === "approved" && (
					...
					>
						...
						<button className="mt-6 bg-white text-black px-6 py-3 rounded-xl font-semibold flex items-center gap-2 hover">
							Bo to Bookings <ArrowRight size={16} />
						</button>
					</motion.div>
				)}
			<PartnerEarning />
			...
			<PricingModel
				...
				data={vehicleData}
				onSuccess={handleGetPricingData}
			/>
