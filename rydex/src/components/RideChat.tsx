...
function RideChat({ currentRole, bookingId, userName, driverName }: any) {
	...
	return (
		<div className="flex flex-col h-full min-h-0 bg-white rounded-2xl overflow-hidden border border-zinc-100 relative">
			...
			<div
				className="flex-1 h-0 overflow-y-auto px-4 py-4 space-y-3 bg-zinc-50"
				...
				{messages.length > 0 &&
					messages.map((m, i) => {
						...
						return (
							...
							>
								<div
									className={`max-w-[72%] px-3.5 py-2.5 text-sm leading-relaxed rounded-2xl shadow-sm ${isMe ? "bg-zinc-950 text-white rounded-br-sm" : "bg-white border border-zinc-200 text-zinc-900 rounded-bl-sm"}`}
								>
									<p className="break-words">{m.text}</p>
									<span className={`text-[8px] block mt-1 ${isMe ? "text-zinc-400 text-right" : "text-zinc-500"}`}>
			...
			<AnimatePresence>
				{showAI && messages.length > 0 && (
					<motion.div
						...
						className="absolute bottom-[68px] flex-shrink-0 overflow-hidden border-t border-zinc-100 bg-white"
					>
						<div className="px-4 pt-3 pb-2">
							...
							{aiLoading ? (
								...
								<div className="flex flex-col gap-1.5">
									...
									<button onClick={()=>getAISuggestions()} className="text-[11px] text-violet-500 hover:text-violet-700 font-semibold text-center mt-1 transition-colors cursor-pointer">
										Refresh Suggestions
									</button>
								</div>
							)}
