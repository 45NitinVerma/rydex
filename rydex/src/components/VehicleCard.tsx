...
function VehicleCard({
	...
	if (vehicle.baseFare && vehicle.pricePerKM && distance) {
		estimated = Math.round(
			vehicle.baseFare + vehicle.pricePerKM * distance,
		);
	}
	...
	return (
		...
		>
			...
			<div className="bg-zinc-100 h-px" />
			...
			<div className="flex flex-col flex-1 p-5 gap-4">
				...
				<div className="grid grid-cols-2 gap-2">
					<div className="bg-zinc-50 border border-zinc-100 rounded-2xl px-3.5 py-3">
						...
						<p className="flex items-center text-zinc-900 text-sm font-black">
							<IndianRupee size={11} />
							{vehicle.pricePerKM}
						</p>
					</div>
				...
				<div className="flex items-end justify-between pt-3 border-t border-zinc-100">
					...
					<motion.button
                        ...
                        onClick={() => onBook()}
						className="flex items-center justify-center gap-2 bg-zinc-900 hover:bg-black text-white text-sm font-black px-6 py-3.5 rounded-2xl transition-colors shadow-md"
					>
						Book
                        <motion.div
                        initial={{x: 0}}
